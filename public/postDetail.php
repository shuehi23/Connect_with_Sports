    ...
    if (!empty($_POST['submit'])) {
        ...
        if (empty($err_msg)) {
            ...
            try {
                $dbh = dbConnect();
                $sql = 'INSERT INTO partnerComment SET send_date=?, from_user=?, comment=?, create_date=?, article_id=?';
                $data = array(date('Y-m-d H:i:s'), $_SESSION['user_id'], $selfcom, date('Y-m-d H:i:s'), $a_id);
    ...
    $partnerDataCmt = getPartnerComment($a_id);
    debug('メッセージ情報:' .print_r($partnerDataCmt, true));
    // DBから投稿情報を取得
    debug($a_id);
    $viewData = getPostOne($a_id);
    debug('写真ID：' . $a_id);
                    ...
                    <div>
                        <div class="poster-wrap">
                            ...
                            <div class="flex" style="flex-direction:column;">
                                <h3 style="font-size:30px;text-align:center;" class="raf">poster</h3>
                                <div class="poster-img"><img src="<?php echo showProfImg(sanitize($getUser['pic'])); ?>" alt=""></div>
                        ...
                        <p><a class="raf" style="font-size:20px;" href="postList.php">
                                << back</a> </p> </div> <div class="detail-wrap">
                                    <div class="detail-title">
                                        <h2 class="raf">title -<span class="min" style="font-size:30px;margin-left:10px"><?php echo $viewData['title']; ?></span><i class="fas fa-heart js-like <?php if (isLike($_SESSION['user_id'], $viewData['id'])) echo 'active'; ?>" data-articleid="<?php echo sanitize($viewData['id']); ?>"></i></h2>
                                    </div>
                                    ...
                                    <div class="sub-title">
                                        <div class="key-title">
                                            ...
                                            <span class="txt"><?php echo sanitize($viewData['key2']); ?></span>
                                            <span class="txt"><?php echo sanitize($viewData['key3']); ?></span>
                                        </div>
                                        <div class="cate-title">
                                            <span class="title min">カテゴリー</span>
                                            <span class="txt"><?php echo sanitize($viewData['c_name']); ?></span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="word-wrap auto-resize">
                                        <ul>
                                            <?php foreach ($partnerDataCmt as $key => $val) : ?>
                                                <li>
                                                    <dl class="flex">
                                                        <dt class="u_pic"><img src="<?php echo showProfImg(sanitize($val['pic'])); ?>" alt=""></dt>
                                                        <dt class="u_name"><?php echo $val['username']; ?></dt>
                                                        <dd class="u_com"><?php echo $val['comment']; ?></dd>
                                                    </dl>
                                                </li>
                                            <?php endforeach; ?>
                                        </ul>
                                    ...
                                    <form class="self-com min" action="" method="post">
                                            <textarea name="selfcom" id="" cols="30" rows="2" class="raf" placeholder="leave a comment"></textarea>
