...
function getPostOne($p_id)
{
    ...
    try {
        $dbh = dbConnect();
        $sql = 'SELECT p.id, p.title, p.place, p.key1, p.key2, p.key3, p.category_id, p.comment_id, p.pic1, p.pic2, p.pic3, p.pic4, p.create_date, p.update_date, p.posted_id, u.username, u.intro, u.pic, c.name AS c_name FROM photo AS p INNER JOIN users AS u ON p.posted_id = u.id INNER JOIN category AS c ON p.category_id = c.id WHERE p.id=? AND p.delete_flg = 0 AND u.delete_flg = 0';
        $data = array($p_id);
        $stmt = queryPost($dbh, $sql, $data);
        if ($stmt) {
            return $stmt->fetch(PDO::FETCH_ASSOC);
...
function getPartnerComment($p_id) {
    ...
    try {
        $dbh = dbConnect();
        $sql = 'SELECT p.from_user, p.comment, u.id, u.user_name, u.pic FROM partnerComment AS p INNER JOIN users AS u ON p.from_user = u.id WHERE photo_id = ? AND p.delete_flg = 0 AND u.delete_flg = 0 ORDER BY p.send_date ASC';
        $data = array($p_id);
